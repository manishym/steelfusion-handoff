...
$VASnapCommon::granite_snapshot_name = "granite_snapshot";
...
sub locate_vms() {
    ...
    for (keys %$lun_ds_hash) {
        ...
        my ($vms, $total_vms_without_filter) =
            get_vms_on_datastore($ds, $include_filter, $exclude_filter);
        $total_vms_on_all_ds += $total_vms_without_filter;
        $ds_vm_hash{$_} = {ds=>$ds, vms => $vms};
    }
    return (\%ds_vm_hash, $total_vms_on_all_ds);
}
...
sub get_vms_on_datastore {
    ...
    my $total_vms_without_filters = 0;
    my $vm_views;
    my $log = LogHandle->new("get_vms");
    if (defined(@$vms_on_ds)) {
        $total_vms_without_filters = scalar(@$vms_on_ds);
    }
    foreach (@$vms_on_ds) {
    ...
    return ($vm_views, $total_vms_without_filters);
}
